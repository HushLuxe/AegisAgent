"""
🛡️ AegisAgent Forensic Engine V5 - SURGICAL
========================================
Professional version for identifying:
- DEX Liquidity & Stress Analysis
- Token Holder Clustering (Forensics)
- Volume/Price Action (VPA)
- Technical Momentum (RSI/BB)
"""
...
class ForensicEngineV5:
    ...
    def analyze(self, data: Dict[str, Any]) -> ForensicReportV5:
        ...
        report.symbol = data.get("symbol_override") or base_token.get("symbol", "") or pool.get("symbol", "UNKNOWN")
    ...
    def _build_structure_narrative(self, r: ForensicReportV5) -> str:
        bf = r.bull_flag
        if not bf.detected:
            return "No significant breakout patterns identified."
