...
OUTPUT        = ROOT_DIR / "frontend" / "public" / "memory.json"
...
def run():
    ...
    # Merge AI Synthesis if exists
    synth_path = Path("/tmp/aegis-agent/synthesis.json")
    if synth_path.exists():
        try:
            synth_data = json.loads(synth_path.read_text())
            out["raw_report"] = synth_data.get("raw_report", "")
            out["seal"] = synth_data.get("seal")
            print(f"[OK] AI Narrative merged from {synth_path}")
        except:
            print("[WARN] Erreur lecture synthesis.json")
